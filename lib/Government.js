...
function Government({ navigation }) {
    var img = [];
    ...
    users.map(user => {
        img.push(
            ...
            <TouchableOpacity style={{width:360,height:275,backgroundColor:'white',marginBottom:10,borderTopLeftRadius:35,borderRadius:35}}
                    onPress={()=>{navigation.push('GovDetails',{user})
                }}>
    ...
    return(
        ...
        <ScrollView >
            <Text style={{paddingBottom:15,textAlign:'center',color:'black',marginTop:15,fontSize:32}}>Government Jobs</Text>
            {img}
            </ScrollView>
            </View>
    );
